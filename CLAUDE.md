...
- **Paged card grids (no scrolling):** the scouting target picker, the deck view, and
  the Collection render fixed pages navigated by `pagerHTML` (chevron arrows +
  clickable dots) and `wirePagedGrid` (swipe left/right; the tap ending a swipe is
  swallowed via `STATE._swipedAt` so it can never pick a card). `gotoPage` re-renders
  through the overlay router (`data-pgact`/`data-pg`). Cards per page adapt via
  `isShortView()` (real height <= 470px, matching the CSS tier; a transient 0-height
  window never reads as short): picker 8 (4x2) or 4, deck 18 (9x2) or 8, Collection
  14 (7x2) or 7. The Collection also shows ONE group at a time behind pill tabs
  (`data-coltab`: Coaches / Salami / Front Office / Skip Tags, each with live x/y
  counts). Page flips slide the grid (`pg-in-l/r`, scaled by `--gs`); the per-card
  deal-in only plays on first open.
